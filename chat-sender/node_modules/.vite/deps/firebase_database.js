import { Component, ComponentContainer, Deferred, LogLevel, Logger, Provider, SDK_VERSION, Sha1, _getProvider, _isFirebaseServerApp, _registerComponent, assert, assertionError, base64, base64Encode, contains, createMockUserToken, deepCopy, deepEqual, errorPrefix, getApp, getDefaultEmulatorHostnameAndPort, getModularInstance, isAdmin, isCloudWorkstation, isEmpty, isMobileCordova, isNodeSdk, isReactNative, isValidFormat, jsonEval, map, pingServer, querystring, registerVersion, safeGet, stringLength, stringToByteArray, stringify, updateEmulatorBanner } from "./index.esm-q1UaBPHY.js";
...
* @internal
*/
function setSDKVersion(version$1) {
	SDK_VERSION$1 = version$1;
}
/**
* @license
...
var DOMStorageWrapper = class {
	...
	*/
	...
	prefixedName_(name$1) {
		return this.prefix_ + name$1;
	}
	toString() {
...
var sha1 = function(str) {
	...
	const sha1$1 = new Sha1();
	sha1$1.update(utf8Bytes);
	const sha1Bytes = sha1$1.digest();
...
*/
function errorForServerCode(code, query$1) {
	...
	const error$1 = /* @__PURE__ */ new Error(code + " at " + query$1._path.toString() + ": " + reason);
	error$1.code = code.toUpperCase();
	return error$1;
}
/**
...
var FirebaseAuthTokenProvider = class {
	...
	getToken(forceRefresh) {
		...
		return this.auth_.getToken(forceRefresh).catch((error$1) => {
			if (error$1 && error$1.code === "auth/token-not-initialized") {
				log("Got auth/token-not-initialized error.  Treating as null token.");
				return null;
			} else return Promise.reject(error$1);
		});
	}
	addTokenChangeListener(listener) {
...
var RepoInfo = class {
	...
	*/
	...
	toURLString() {
		...
		const query$1 = this.includeNamespaceInQueryParams ? `?ns=${this.namespace}` : "";
		return `${protocol}${this.host}/${query$1}`;
	}
};
...
var StatsCollection = class {
	...
	incrementCounter(name$1, amount = 1) {
		if (!contains(this.counters_, name$1)) this.counters_[name$1] = 0;
		this.counters_[name$1] += amount;
	}
	get() {
...
var BrowserPollConnection = class BrowserPollConnection {
	...
	*/
	open(onMessage, onDisconnect$1) {
		this.curSegmentNum = 0;
		this.onDisconnect_ = onDisconnect$1;
...
var FirebaseIFrameScriptHolder = class FirebaseIFrameScriptHolder {
	...
	*/
	constructor(commandCB, onMessageCB, onDisconnect$1, urlFn) {
		this.onDisconnect = onDisconnect$1;
		this.urlFn = urlFn;
	...
	*/
	close() {
		...
		const onDisconnect$1 = this.onDisconnect;
		if (onDisconnect$1) {
	...
	*/
	open(onMessage, onDisconnect$1) {
		...
		try {
			...
			const error$1 = e.message || e.data;
			if (error$1) this.log_(error$1);
			this.onClosed_();
			return;
		}
		this.mySock.onopen = () => {
	...
	*/
	onHandshake_(handshake) {
		...
		if (this.state_ === 0) {
			...
			if (PROTOCOL_VERSION !== version$1) warn("Protocol version mismatch detected");
	...
	startUpgrade_(conn) {
		...
		const onDisconnect$1 = this.disconnReceiver_(this.secondaryConn_);
		this.secondaryConn_.open(onMessage, onDisconnect$1);
...
function validationPathPush(validationPath, child$1) {
	if (validationPath.parts_.length > 0) validationPath.byteLength_ += 1;
	validationPath.parts_.push(child$1);
	validationPath.byteLength_ += stringLength(child$1);
...
var PersistentConnection = class PersistentConnection extends ServerActions {
	...
	*/
	...
	get(query$1) {
		...
		const outstandingGet = {
			action: "g",
			request: {
				p: query$1._path.toString(),
				q: query$1._queryObject
			},
			onComplete: (message) => {
	...
	listen(query$1, currentHashFn, tag, onComplete) {
		this.initConnection_();
		const queryId = query$1._queryIdentifier;
		const pathString = query$1._path.toString();
		this.log_("Listen called for " + pathString + " " + queryId);
		if (!this.listens.has(pathString)) this.listens.set(pathString, /* @__PURE__ */ new Map());
		assert(query$1._queryParams.isDefault() || !query$1._queryParams.loadsAllData(), "listen() called for non-default but complete query");
	...
	sendGet_(index) {
		const get$1 = this.outstandingGets_[index];
		this.sendRequest("g", get$1.request, (message) => {
			delete this.outstandingGets_[index];
			this.outstandingGetCount_--;
			if (this.outstandingGetCount_ === 0) this.outstandingGets_ = [];
			if (get$1.onComplete) get$1.onComplete(message);
		});
	}
	sendListen_(listenSpec) {
		const query$1 = listenSpec.query;
		const pathString = query$1._path.toString();
		const queryId = query$1._queryIdentifier;
		...
		this.sendRequest(action, req, (message) => {
			...
			PersistentConnection.warnOnListenWarnings_(payload, query$1);
	...
	static warnOnListenWarnings_(payload, query$1) {
		if (payload && typeof payload === "object" && contains(payload, "w")) {
			const warnings = safeGet(payload, "w");
			if (Array.isArray(warnings) && ~warnings.indexOf("no_index")) {
				const indexSpec = "\".indexOn\": \"" + query$1._queryParams.getIndex().toString() + "\"";
				const indexPath = query$1._path.toString();
	...
	* @inheritDoc
	*/
	unlisten(query$1, tag) {
		const pathString = query$1._path.toString();
		const queryId = query$1._queryIdentifier;
		this.log_("Unlisten called for " + pathString + " " + queryId);
		assert(query$1._queryParams.isDefault() || !query$1._queryParams.loadsAllData(), "unlisten() called for non-default but complete query");
		if (this.removeListen_(pathString, queryId) && this.connected_) this.sendUnlisten_(pathString, queryId, query$1._queryObject, tag);
	}
	...
	async establishConnection_() {
		if (this.shouldReconnect_()) {
			...
			const onDisconnect$1 = this.onRealtimeDisconnect_.bind(this);
			...
			try {
				...
				if (!canceled) {
					...
					connection = new Connection(connId, this.repoInfo_, this.applicationId_, this.appCheckToken_, this.authToken_, onDataMessage, onReady, onDisconnect$1, (reason) => {
						warn(reason + " (" + this.repoInfo_.toString() + ")");
						this.interrupt(SERVER_KILL_INTERRUPT_REASON);
					}, lastSessionId);
				} else log("getToken() completed but was canceled");
			} catch (error$1) {
				this.log_("Failed to get token: " + error$1);
				if (!canceled) {
					if (this.repoInfo_.nodeAdmin) warn(error$1);
					closeFn();
				}
			}
		}
	...
	onListenRevoked_(pathString, query$1) {
		let queryId;
		if (!query$1) queryId = "default";
		else queryId = query$1.map((q) => ObjectToUniqueKey(q)).join("$");
	...
	removeListen_(pathString, queryId) {
		...
		if (this.listens.has(normalizedPathString)) {
			const map$1 = this.listens.get(normalizedPathString);
			listen = map$1.get(queryId);
			map$1.delete(queryId);
			if (map$1.size === 0) this.listens.delete(normalizedPathString);
		} else listen = void 0;
		return listen;
	}
...
var NamedNode = class NamedNode {
	...
	static Wrap(name$1, node) {
		return new NamedNode(name$1, node);
	}
};
/**
* @license
...
var PriorityIndex = class extends Index {
	...
	makePost(indexValue, name$1) {
		...
		return new NamedNode(name$1, new LeafNode("[PRIORITY-POST]", priorityNode));
	}
	/**
	* @returns String representation for inclusion in a query spec
	*/
...
var buildChildSet = function(childList, cmp, keyFn, mapSortFn) {
	...
	const buildFrom12Array = function(base12$1) {
		let node = null;
		let root$1 = null;
		...
		for (let i = 0; i < base12$1.count; ++i) {
			const isOne = base12$1.nextBitIsOne();
			const chunkSize = Math.pow(2, base12$1.count - (i + 1));
	...
	*/
	...
	getImmediateChild(childName) {
		if (childName === ".priority") return this.getPriority();
			...
			const child$1 = this.children_.get(childName);
			return child$1 === null ? EMPTY_NODE : child$1;
		}
	}
	/** @inheritDoc */
	getChild(path) {
...
function nodeFromJSON(json, priority = null) {
	...
	if (!(json instanceof Array) && USE_HINZE) {
		...
		each(json, (key, child$1) => {
			if (key.substring(0, 1) !== ".") {
				const childNode = nodeFromJSON(child$1);
...
var ValueIndex = class extends Index {
	...
	makePost(indexValue, name$1) {
		const valueNode = nodeFromJSON(indexValue);
		return new NamedNode(name$1, valueNode);
	}
	/**
	* @returns String representation for inclusion in a query spec
	*/
...
var ReadonlyRestClient = class ReadonlyRestClient extends ServerActions {
	...
	static getListenId_(query$1, tag) {
		if (tag !== void 0) return "tag$" + tag;
		else {
			assert(query$1._queryParams.isDefault(), "should have a tag if it's not a default query.");
			return query$1._path.toString();
		}
	}
	/**
	* @param repoInfo_ - Data about the namespace we are connecting to
	...
	*/
	...
	listen(query$1, currentHashFn, tag, onComplete) {
		const pathString = query$1._path.toString();
		this.log_("Listen called for " + pathString + " " + query$1._queryIdentifier);
		const listenId = ReadonlyRestClient.getListenId_(query$1, tag);
		const thisListen = {};
		this.listens_[listenId] = thisListen;
		const queryStringParameters = queryParamsToRestQueryStringParameters(query$1._queryParams);
		this.restRequest_(pathString + ".json", queryStringParameters, (error$1, result) => {
			let data = result;
			if (error$1 === 404) {
				data = null;
				error$1 = null;
			}
			if (error$1 === null) this.onDataUpdate_(pathString, data, false, tag);
			if (safeGet(this.listens_, listenId) === thisListen) {
				let status;
				if (!error$1) status = "ok";
				else if (error$1 === 401) status = "permission_denied";
				else status = "rest_error:" + error$1;
...
function sparseSnapshotTreeRemember(sparseSnapshotTree, path, data) {
	if (pathIsEmpty(path)) {
		...
		const child$1 = sparseSnapshotTree.children.get(childKey);
		path = pathPopFront(path);
		sparseSnapshotTreeRemember(child$1, path, data);
	}
}
/**
...
(function(OperationType$1) {
	OperationType$1[OperationType$1["OVERWRITE"] = 0] = "OVERWRITE";
	OperationType$1[OperationType$1["MERGE"] = 1] = "MERGE";
	OperationType$1[OperationType$1["ACK_USER_WRITE"] = 2] = "ACK_USER_WRITE";
	OperationType$1[OperationType$1["LISTEN_COMPLETE"] = 3] = "LISTEN_COMPLETE";
	...
	*/
	findRootMostMatchingPathAndValue(relativePath, predicate) {
		if (this.value != null && predicate(this.value)) return {
			...
			const child$1 = this.children.get(front);
			if (child$1 !== null) {
				const childExistingPathAndValue = child$1.findRootMostMatchingPathAndValue(pathPopFront(relativePath), predicate);
	...
	*/
	remove(relativePath) {
		if (pathIsEmpty(relativePath)) if (this.children.isEmpty()) return new ImmutableTree(null);
			...
			const child$1 = this.children.get(front);
			if (child$1) {
				const newChild = child$1.remove(pathPopFront(relativePath));
				let newChildren;
	...
	*/
	get(relativePath) {
		if (pathIsEmpty(relativePath)) return this.value;
			...
			const front = pathGetFront(relativePath);
			const child$1 = this.children.get(front);
			if (child$1) return child$1.get(pathPopFront(relativePath));
			else return null;
		}
	}
	/**
...
function compoundWriteAddWrite(compoundWrite, path, node) {
	if (pathIsEmpty(path)) return new CompoundWrite(new ImmutableTree(node));
		...
		if (rootmost != null) {
			...
			const newWriteTree$1 = compoundWrite.writeTree_.setTree(path, subtree);
			return new CompoundWrite(newWriteTree$1);
		}
	}
}
...
function compoundWriteRemoveWrite(compoundWrite, path) {
	if (pathIsEmpty(path)) return CompoundWrite.empty();
		...
		const newWriteTree$1 = compoundWrite.writeTree_.setTree(path, new ImmutableTree(null));
		return new CompoundWrite(newWriteTree$1);
	}
}
/**
			...
			if (write.snap) {
				...
				if (pathContains(treeRoot, writePath)) {
					...
					if (pathIsEmpty(relativePath)) compoundWrite = compoundWriteAddWrites(compoundWrite, newEmptyPath(), write.children);
						...
						const child$1 = safeGet(write.children, pathGetFront(relativePath));
						if (child$1) {
							const deepNode = child$1.getChild(pathPopFront(relativePath));
...
function writeTreeCalcCompleteChild(writeTree, treePath, childKey, existingServerSnap) {
	...
	if (shadowingNode != null) return shadowingNode;
	else if (existingServerSnap.isCompleteForChild(childKey)) {
		const childMerge = compoundWriteChildCompoundWrite(writeTree.visibleWrites, path);
...
var NoCompleteChildSource_ = class {
	...
	getChildAfterChild(index, child$1, reverse) {
		return null;
	}
};
/**
...
var WriteTreeCompleteChildSource = class {
	...
	getChildAfterChild(index, child$1, reverse) {
		...
		const nodes = writeTreeRefCalcIndexedSlice(this.writes_, completeServerData, child$1, 1, reverse, index);
...
function viewProcessorApplyOperation(viewProcessor, oldViewCache, operation, writesCache, completeCache) {
	...
	let newViewCache$1, filterServerNode;
	if (operation.type === OperationType.OVERWRITE) {
		const overwrite = operation;
		if (overwrite.source.fromUser) newViewCache$1 = viewProcessorApplyUserOverwrite(viewProcessor, oldViewCache, overwrite.path, overwrite.snap, writesCache, completeCache, accumulator);
		else {
			assert(overwrite.source.fromServer, "Unknown source.");
			filterServerNode = overwrite.source.tagged || oldViewCache.serverCache.isFiltered() && !pathIsEmpty(overwrite.path);
			newViewCache$1 = viewProcessorApplyServerOverwrite(viewProcessor, oldViewCache, overwrite.path, overwrite.snap, writesCache, completeCache, filterServerNode, accumulator);
		}
	} else if (operation.type === OperationType.MERGE) {
		const merge = operation;
		if (merge.source.fromUser) newViewCache$1 = viewProcessorApplyUserMerge(viewProcessor, oldViewCache, merge.path, merge.children, writesCache, completeCache, accumulator);
		else {
			assert(merge.source.fromServer, "Unknown source.");
			filterServerNode = merge.source.tagged || oldViewCache.serverCache.isFiltered();
			newViewCache$1 = viewProcessorApplyServerMerge(viewProcessor, oldViewCache, merge.path, merge.children, writesCache, completeCache, filterServerNode, accumulator);
		}
	} else if (operation.type === OperationType.ACK_USER_WRITE) {
		const ackUserWrite = operation;
		if (!ackUserWrite.revert) newViewCache$1 = viewProcessorAckUserWrite(viewProcessor, oldViewCache, ackUserWrite.path, ackUserWrite.affectedTree, writesCache, completeCache, accumulator);
		else newViewCache$1 = viewProcessorRevertUserWrite(viewProcessor, oldViewCache, ackUserWrite.path, writesCache, completeCache, accumulator);
	} else if (operation.type === OperationType.LISTEN_COMPLETE) newViewCache$1 = viewProcessorListenComplete(viewProcessor, oldViewCache, operation.path, writesCache, accumulator);
	else throw assertionError("Unknown operation type: " + operation.type);
	const changes = accumulator.getChanges();
	viewProcessorMaybeAddValueEvent(oldViewCache, newViewCache$1, changes);
	return {
		viewCache: newViewCache$1,
		changes
	};
}
function viewProcessorMaybeAddValueEvent(oldViewCache, newViewCache$1, accumulator) {
	const eventSnap = newViewCache$1.eventCache;
	if (eventSnap.isFullyInitialized()) {
		const isLeafOrEmpty = eventSnap.getNode().isLeafNode() || eventSnap.getNode().isEmpty();
		const oldCompleteSnap = viewCacheGetCompleteEventSnap(oldViewCache);
		if (accumulator.length > 0 || !oldViewCache.eventCache.isFullyInitialized() || isLeafOrEmpty && !eventSnap.getNode().equals(oldCompleteSnap) || !eventSnap.getNode().getPriority().equals(oldCompleteSnap.getPriority())) accumulator.push(changeValue(viewCacheGetCompleteEventSnap(newViewCache$1)));
	}
}
...
function viewProcessorApplyServerOverwrite(viewProcessor, oldViewCache, changePath, changedSnap, writesCache, completeCache, filterServerNode, accumulator) {
	...
	const newViewCache$1 = viewCacheUpdateServerSnap(oldViewCache, newServerCache, oldServerSnap.isFullyInitialized() || pathIsEmpty(changePath), serverFilter.filtersNodes());
	const source = new WriteTreeCompleteChildSource(writesCache, newViewCache$1, completeCache);
	return viewProcessorGenerateEventCacheAfterServerEvent(viewProcessor, newViewCache$1, changePath, writesCache, source, accumulator);
}
function viewProcessorApplyUserOverwrite(viewProcessor, oldViewCache, changePath, changedSnap, writesCache, completeCache, accumulator) {
	const oldEventSnap = oldViewCache.eventCache;
	let newViewCache$1, newEventCache;
	const source = new WriteTreeCompleteChildSource(writesCache, oldViewCache, completeCache);
	if (pathIsEmpty(changePath)) {
		newEventCache = viewProcessor.filter.updateFullNode(oldViewCache.eventCache.getNode(), changedSnap, accumulator);
		newViewCache$1 = viewCacheUpdateEventSnap(oldViewCache, newEventCache, true, viewProcessor.filter.filtersNodes());
	} else {
		const childKey = pathGetFront(changePath);
		if (childKey === ".priority") {
			newEventCache = viewProcessor.filter.updatePriority(oldViewCache.eventCache.getNode(), changedSnap);
			newViewCache$1 = viewCacheUpdateEventSnap(oldViewCache, newEventCache, oldEventSnap.isFullyInitialized(), oldEventSnap.isFiltered());
		} else {
			const childChangePath = pathPopFront(changePath);
			...
			if (!oldChild.equals(newChild)) {
				...
				newViewCache$1 = viewCacheUpdateEventSnap(oldViewCache, newEventSnap, oldEventSnap.isFullyInitialized(), viewProcessor.filter.filtersNodes());
			} else newViewCache$1 = oldViewCache;
		}
	}
	return newViewCache$1;
}
...
function viewProcessorAckUserWrite(viewProcessor, viewCache, ackPath, affectedTree, writesCache, completeCache, accumulator) {
	...
	if (affectedTree.value != null) if (pathIsEmpty(ackPath) && serverCache.isFullyInitialized() || serverCache.isCompleteForPath(ackPath)) return viewProcessorApplyServerOverwrite(viewProcessor, viewCache, ackPath, serverCache.getNode().getChild(ackPath), writesCache, completeCache, filterServerNode, accumulator);
		...
		serverCache.getNode().forEachChild(KEY_INDEX, (name$1, node) => {
			changedChildren = changedChildren.set(new Path(name$1), node);
		});
...
function viewProcessorListenComplete(viewProcessor, viewCache, path, writesCache, accumulator) {
	...
	const newViewCache$1 = viewCacheUpdateServerSnap(viewCache, oldServerNode.getNode(), oldServerNode.isFullyInitialized() || pathIsEmpty(path), oldServerNode.isFiltered());
	return viewProcessorGenerateEventCacheAfterServerEvent(viewProcessor, newViewCache$1, path, writesCache, NO_COMPLETE_CHILD_SOURCE, accumulator);
}
...
*/
function syncPointGetView(syncPoint, query$1, writesCache, serverCache, serverCacheComplete) {
	const queryId = query$1._queryIdentifier;
	const view = syncPoint.views.get(queryId);
	if (!view) {
		...
		return new View(query$1, viewCache);
	}
	return view;
}
/**
...
* @returns Events to raise.
*/
function syncPointAddEventRegistration(syncPoint, query$1, eventRegistration, writesCache, serverCache, serverCacheComplete) {
	const view = syncPointGetView(syncPoint, query$1, writesCache, serverCache, serverCacheComplete);
	if (!syncPoint.views.has(query$1._queryIdentifier)) syncPoint.views.set(query$1._queryIdentifier, view);
...
*/
function syncPointRemoveEventRegistration(syncPoint, query$1, eventRegistration, cancelError) {
	const queryId = query$1._queryIdentifier;
	const removed = [];
	let cancelEvents = [];
	...
	if (hadCompleteView && !syncPointHasCompleteView(syncPoint)) removed.push(new (syncPointGetReferenceConstructor())(query$1._repo, query$1._path));
	return {
		removed,
		events: cancelEvents
	};
...
function syncPointViewForQuery(syncPoint, query$1) {
	if (query$1._queryParams.loadsAllData()) return syncPointGetCompleteView(syncPoint);
	else {
		const queryId = query$1._queryIdentifier;
		return syncPoint.views.get(queryId);
	}
}
function syncPointViewExistsForQuery(syncPoint, query$1) {
	return syncPointViewForQuery(syncPoint, query$1) != null;
}
...
* @returns Cancel events, if cancelError was provided.
*/
function syncTreeRemoveEventRegistration(syncTree, query$1, eventRegistration, cancelError, skipListenerDedup = false) {
	const path = query$1._path;
	const maybeSyncPoint = syncTree.syncPointTree_.get(path);
	let cancelEvents = [];
	if (maybeSyncPoint && (query$1._queryIdentifier === "default" || syncPointViewExistsForQuery(maybeSyncPoint, query$1))) {
		const removedAndEvents = syncPointRemoveEventRegistration(maybeSyncPoint, query$1, eventRegistration, cancelError);
		...
		if (!skipListenerDedup) {
			...
			* properly set up.
			*/
			const removingDefault = -1 !== removed.findIndex((query$2) => {
				return query$2._queryParams.loadsAllData();
			});
			...
			if (!covered && removed.length > 0 && !cancelError) if (removingDefault) syncTree.listenProvider_.stopListening(syncTreeQueryForListening_(query$1), null);
...
* @returns Events to raise.
*/
function syncTreeAddEventRegistration(syncTree, query$1, eventRegistration, skipSetupListener = false) {
	const path = query$1._path;
	let serverCache = null;
	...
	const viewAlreadyExists = syncPointViewExistsForQuery(syncPoint, query$1);
	if (!viewAlreadyExists && !query$1._queryParams.loadsAllData()) {
		const queryKey = syncTreeMakeQueryKey_(query$1);
	...
	let events = syncPointAddEventRegistration(syncPoint, query$1, eventRegistration, writesCache, serverCache, serverCacheComplete);
	if (!viewAlreadyExists && !foundAncestorDefaultView && !skipSetupListener) {
		const view = syncPointViewForQuery(syncPoint, query$1);
		events = events.concat(syncTreeSetupListener_(syncTree, query$1, view));
	}
	return events;
}
/**
...
function syncTreeCalcCompleteEventCache(syncTree, path, writeIdsToExclude) {
	...
	const serverCache = syncTree.syncPointTree_.findOnPath(path, (pathSoFar, syncPoint) => {
		...
		const serverCache$1 = syncPointGetCompleteServerCache(syncPoint, relativePath);
		if (serverCache$1) return serverCache$1;
	});
	return writeTreeCalcCompleteEventCache(writeTree, path, serverCache, writeIdsToExclude, includeHiddenSets);
}
function syncTreeGetServerValue(syncTree, query$1) {
	const path = query$1._path;
	let serverCache = null;
	syncTree.syncPointTree_.foreachOnPath(path, (pathToSyncPoint, sp) => {
		const relativePath = newRelativePath(pathToSyncPoint, path);
	...
	const writesCache = writeTreeChildWrites(syncTree.pendingWriteTree_, query$1._path);
	const view = syncPointGetView(syncPoint, query$1, writesCache, serverCacheComplete ? serverCacheNode.getNode() : ChildrenNode.EMPTY_NODE, serverCacheComplete);
...
function syncTreeCreateListenerForView_(syncTree, view) {
	const query$1 = view.query;
	const tag = syncTreeTagForQuery(syncTree, query$1);
	return {
		hashFn: () => {
			return (viewGetServerCache(view) || ChildrenNode.EMPTY_NODE).hash();
		},
		onComplete: (status) => {
			if (status === "ok") if (tag) return syncTreeApplyTaggedListenComplete(syncTree, query$1._path, tag);
			else return syncTreeApplyListenComplete(syncTree, query$1._path);
			else {
				const error$1 = errorForServerCode(status, query$1);
				return syncTreeRemoveEventRegistration(syncTree, query$1, null, error$1);
			}
		}
	};
}
...
*/
function syncTreeTagForQuery(syncTree, query$1) {
	const queryKey = syncTreeMakeQueryKey_(query$1);
...
*/
function syncTreeMakeQueryKey_(query$1) {
	return query$1._path.toString() + "$" + query$1._queryIdentifier;
}
/**
...
*/
function syncTreeQueryForListening_(query$1) {
	if (query$1._queryParams.loadsAllData() && !query$1._queryParams.isDefault()) return new (syncTreeGetReferenceConstructor())(query$1._repo, query$1._path);
	else return query$1;
}
...
*/
function syncTreeSetupListener_(syncTree, query$1, view) {
	const path = query$1._path;
	const tag = syncTreeTagForQuery(syncTree, query$1);
	const listener = syncTreeCreateListenerForView_(syncTree, view);
	const events = syncTree.listenProvider_.startListening(syncTreeQueryForListening_(query$1), tag, listener.hashFn, listener.onComplete);
	const subtree = syncTree.syncPointTree_.subtree(path);
	if (tag) assert(!syncPointHasCompleteView(subtree.value), "If we're adding a query, it shouldn't be shadowed");
		...
		const queriesToStop = subtree.fold((relativePath, maybeChildSyncPoint, childMap) => {
			if (!pathIsEmpty(relativePath) && maybeChildSyncPoint && syncPointHasCompleteView(maybeChildSyncPoint)) return [syncPointGetCompleteView(maybeChildSyncPoint).query];
				...
				let queries = [];
				if (maybeChildSyncPoint) queries = queries.concat(syncPointGetQueryViews(maybeChildSyncPoint).map((view$1) => view$1.query));
...
var ExistingValueProvider = class ExistingValueProvider {
	...
	getImmediateChild(childName) {
		const child$1 = this.node_.getImmediateChild(childName);
		return new ExistingValueProvider(child$1);
	}
	node() {
		return this.node_;
	}
...
var Tree = class {
	...
	*/
	constructor(name$1 = "", parent = null, node = {
		children: {},
		childCount: 0
	}) {
		this.name = name$1;
...
function treeSubTree(tree, pathObj) {
	...
	let child$1 = tree, next = pathGetFront(path);
	while (next !== null) {
		const childNode = safeGet(child$1.node.children, next) || {
			children: {},
			childCount: 0
		};
		child$1 = new Tree(next, child$1, childNode);
		path = pathPopFront(path);
...
function treeForEachChild(tree, action) {
	each(tree.node.children, (child$1, childTree) => {
		action(new Tree(child$1, tree, childTree));
	});
}
/**
...
function treeForEachDescendant(tree, action, includeSelf, childrenFirst) {
	...
	treeForEachChild(tree, (child$1) => {
		treeForEachDescendant(child$1, action, true, childrenFirst);
	});
	if (includeSelf && childrenFirst) action(tree);
}
/**
...
*/
function treeUpdateChild(tree, childName, child$1) {
	const childEmpty = treeIsEmpty(child$1);
	...
	if (childEmpty && childExists) {
		...
		tree.node.children[childName] = child$1.node;
		tree.node.childCount++;
		treeUpdateParents(tree);
	}
}
...
var validateFirebaseData = function(errorPrefix$1, data, path_) {
	...
	if (data === void 0) throw new Error(errorPrefix$1 + "contains undefined " + validationPathToErrorString(path));
	if (typeof data === "function") throw new Error(errorPrefix$1 + "contains a function " + validationPathToErrorString(path) + " with contents = " + data.toString());
	if (isInvalidJSONNumber(data)) throw new Error(errorPrefix$1 + "contains " + data.toString() + " " + validationPathToErrorString(path));
	if (typeof data === "string" && data.length > MAX_LEAF_SIZE_ / 3 && stringLength(data) > MAX_LEAF_SIZE_) throw new Error(errorPrefix$1 + "contains a string greater than 10485760 utf8 bytes " + validationPathToErrorString(path) + " ('" + data.substring(0, 50) + "...')");
	if (data && typeof data === "object") {
		...
		each(data, (key, value) => {
			if (key === ".value") hasDotValue = true;
				...
				hasActualChild = true;
				if (!isValidKey(key)) throw new Error(errorPrefix$1 + " contains an invalid key (" + key + ") " + validationPathToErrorString(path) + ".  Keys must be non-empty strings and can't contain \".\", \"#\", \"$\", \"/\", \"[\", or \"]\"");
			}
			validationPathPush(path, key);
			validateFirebaseData(errorPrefix$1, value, path);
			validationPathPop(path);
		});
		if (hasDotValue && hasActualChild) throw new Error(errorPrefix$1 + " contains \".value\" child " + validationPathToErrorString(path) + " in addition to actual children.");
	}
};
/**
...
var validateFirebaseMergePaths = function(errorPrefix$1, mergePaths) {
	...
	for (i = 0; i < mergePaths.length; i++) {
		...
		else if (!isValidKey(keys[j])) throw new Error(errorPrefix$1 + "contains an invalid key (" + keys[j] + ") in path " + curPath.toString() + ". Keys must be non-empty strings and can't contain \".\", \"#\", \"$\", \"/\", \"[\", or \"]\"");
	}
	mergePaths.sort(pathCompare);
	let prevPath = null;
	for (i = 0; i < mergePaths.length; i++) {
		curPath = mergePaths[i];
		if (prevPath !== null && pathContains(prevPath, curPath)) throw new Error(errorPrefix$1 + "contains a path " + prevPath.toString() + " that is ancestor of another path " + curPath.toString());
		prevPath = curPath;
	}
};
/**
...
function repoStart(repo, appId, authOverride) {
	...
	repo.infoSyncTree_ = new SyncTree({
		startListening: (query$1, tag, currentHashFn, onComplete) => {
			let infoEvents = [];
			const node = repo.infoData_.getNode(query$1._path);
			if (!node.isEmpty()) {
				infoEvents = syncTreeApplyServerOverwrite(repo.infoSyncTree_, query$1._path, node);
				setTimeout(() => {
					onComplete("ok");
				}, 0);
			}
	...
	repo.serverSyncTree_ = new SyncTree({
		startListening: (query$1, tag, currentHashFn, onComplete) => {
			repo.server_.listen(query$1, currentHashFn, tag, (status, data) => {
				const events = onComplete(status, data);
				eventQueueRaiseEventsForChangedPath(repo.eventQueue_, query$1._path, events);
			});
			return [];
		},
		stopListening: (query$1, tag) => {
			repo.server_.unlisten(query$1, tag);
		}
	});
}
/**
...
*/
function repoGetValue(repo, query$1, eventRegistration) {
	const cached = syncTreeGetServerValue(repo.serverSyncTree_, query$1);
	if (cached != null) return Promise.resolve(cached);
	return repo.server_.get(query$1).then((payload) => {
		const node = nodeFromJSON(payload).withIndex(query$1._queryParams.getIndex());
		/**
		...
		*/
		syncTreeAddEventRegistration(repo.serverSyncTree_, query$1, eventRegistration, true);
		let events;
		if (query$1._queryParams.loadsAllData()) events = syncTreeApplyServerOverwrite(repo.serverSyncTree_, query$1._path, node);
		else {
			const tag = syncTreeTagForQuery(repo.serverSyncTree_, query$1);
			events = syncTreeApplyTaggedQueryOverwrite(repo.serverSyncTree_, query$1._path, node, tag);
		}
		eventQueueRaiseEventsForChangedPath(repo.eventQueue_, query$1._path, events);
		syncTreeRemoveEventRegistration(repo.serverSyncTree_, query$1, eventRegistration, null, true);
		return node;
	}, (err) => {
		repoLog(repo, "get for query " + stringify(query$1) + " failed: " + err);
...
function repoAddEventCallbackForQuery(repo, query$1, eventRegistration) {
	let events;
	if (pathGetFront(query$1._path) === ".info") events = syncTreeAddEventRegistration(repo.infoSyncTree_, query$1, eventRegistration);
	else events = syncTreeAddEventRegistration(repo.serverSyncTree_, query$1, eventRegistration);
	eventQueueRaiseEventsAtPath(repo.eventQueue_, query$1._path, events);
}
function repoRemoveEventCallbackForQuery(repo, query$1, eventRegistration) {
	let events;
	if (pathGetFront(query$1._path) === ".info") events = syncTreeRemoveEventRegistration(repo.infoSyncTree_, query$1, eventRegistration);
	else events = syncTreeRemoveEventRegistration(repo.serverSyncTree_, query$1, eventRegistration);
	eventQueueRaiseEventsAtPath(repo.eventQueue_, query$1._path, events);
}
...
function repoAggregateTransactionQueuesForNode(repo, node, queue) {
	...
	treeForEachChild(node, (child$1) => {
		repoAggregateTransactionQueuesForNode(repo, child$1, queue);
	});
}
/**
...
var DataEvent = class {
	...
	*/
	...
	getPath() {
		const ref$1 = this.snapshot.ref;
		if (this.eventType === "value") return ref$1._path;
		else return ref$1.parent._path;
	}
	getEventType() {
...
var CancelEvent = class {
	constructor(eventRegistration, error$1, path) {
		this.eventRegistration = eventRegistration;
		this.error = error$1;
		this.path = path;
	}
...
var CallbackContext = class {
	...
	onCancel(error$1) {
		...
		return this.cancelCallback.call(null, error$1);
	}
...
*/
function validateNoPreviousOrderByCall(query$1, fnName) {
	if (query$1._orderByCalled === true) throw new Error(fnName + ": You can't combine multiple orderBy calls.");
}
/**
...
var ReferenceImpl = class ReferenceImpl extends QueryImpl {
	...
	get root() {
		let ref$1 = this;
		while (ref$1.parent !== null) ref$1 = ref$1.parent;
		return ref$1;
	}
};
/**
...
var DataSnapshot = class DataSnapshot {
	...
	* @hideconstructor
	*/
	constructor(_node, ref$1, _index) {
		this._node = _node;
		this.ref = ref$1;
		this._index = _index;
	}
...
*/
function onDisconnect(ref$1) {
	ref$1 = getModularInstance(ref$1);
	return new OnDisconnect(ref$1._repo, ref$1._path);
}
/**
...
function push(parent, value) {
	...
	const name$1 = nextPushId(now);
	const thenablePushRef = child(parent, name$1);
	const pushRef = child(parent, name$1);
	let promise;
...
*/
function remove(ref$1) {
	validateWritablePath("remove", ref$1._path);
	return set(ref$1, null);
}
/**
...
*/
function set(ref$1, value) {
	ref$1 = getModularInstance(ref$1);
	validateWritablePath("set", ref$1._path);
	validateFirebaseDataArg("set", value, ref$1._path, false);
	const deferred = new Deferred();
	repoSetWithPriority(ref$1._repo, ref$1._path, value, null, deferred.wrapCallback(() => {}));
	return deferred.promise;
}
/**
...
* @returns Resolves when write to server is complete.
*/
function setPriority(ref$1, priority) {
	ref$1 = getModularInstance(ref$1);
	validateWritablePath("setPriority", ref$1._path);
	validatePriority("setPriority", priority, false);
	const deferred = new Deferred();
	repoSetWithPriority(ref$1._repo, pathChild(ref$1._path, ".priority"), priority, null, deferred.wrapCallback(() => {}));
	return deferred.promise;
}
/**
...
* @returns Resolves when write to server is complete.
*/
function setWithPriority(ref$1, value, priority) {
	validateWritablePath("setWithPriority", ref$1._path);
	validateFirebaseDataArg("setWithPriority", value, ref$1._path, false);
	validatePriority("setWithPriority", priority, false);
	if (ref$1.key === ".length" || ref$1.key === ".keys") throw "setWithPriority failed: " + ref$1.key + " is a read-only object.";
	const deferred = new Deferred();
	repoSetWithPriority(ref$1._repo, ref$1._path, value, priority, deferred.wrapCallback(() => {}));
	return deferred.promise;
}
/**
...
*/
function update(ref$1, values) {
	validateFirebaseMergeDataArg("update", values, ref$1._path, false);
	const deferred = new Deferred();
	repoUpdate(ref$1._repo, ref$1._path, values, deferred.wrapCallback(() => {}));
	return deferred.promise;
}
/**
...
*/
function get(query$1) {
	query$1 = getModularInstance(query$1);
	const callbackContext = new CallbackContext(() => {});
	const container = new ValueEventRegistration(callbackContext);
	return repoGetValue(query$1._repo, query$1, container).then((node) => {
		return new DataSnapshot(node, new ReferenceImpl(query$1._repo, query$1._path), query$1._queryParams.getIndex());
	});
}
/**
...
var ValueEventRegistration = class ValueEventRegistration {
	...
	createEvent(change, query$1) {
		const index = query$1._queryParams.getIndex();
		return new DataEvent("value", this, new DataSnapshot(change.snapshotNode, new ReferenceImpl(query$1._repo, query$1._path), index));
	}
	...
	createCancelEvent(error$1, path) {
		if (this.callbackContext.hasCancelCallback) return new CancelEvent(this, error$1, path);
		else return null;
	}
	matches(other) {
...
var ChildEventRegistration = class ChildEventRegistration {
	...
	createEvent(change, query$1) {
		...
		const childRef = child(new ReferenceImpl(query$1._repo, query$1._path), change.childName);
		const index = query$1._queryParams.getIndex();
...
function addEventListener(query$1, eventType, callback, cancelCallbackOrListenOptions, options) {
	let cancelCallback;
	...
	if (options && options.onlyOnce) {
		...
		const onceCallback = (dataSnapshot, previousChildName) => {
			repoRemoveEventCallbackForQuery(query$1._repo, query$1, container);
	...
	repoAddEventCallbackForQuery(query$1._repo, query$1, container);
	return () => repoRemoveEventCallbackForQuery(query$1._repo, query$1, container);
}
function onValue(query$1, callback, cancelCallbackOrListenOptions, options) {
	return addEventListener(query$1, "value", callback, cancelCallbackOrListenOptions, options);
}
function onChildAdded(query$1, callback, cancelCallbackOrListenOptions, options) {
	return addEventListener(query$1, "child_added", callback, cancelCallbackOrListenOptions, options);
}
function onChildChanged(query$1, callback, cancelCallbackOrListenOptions, options) {
	return addEventListener(query$1, "child_changed", callback, cancelCallbackOrListenOptions, options);
}
function onChildMoved(query$1, callback, cancelCallbackOrListenOptions, options) {
	return addEventListener(query$1, "child_moved", callback, cancelCallbackOrListenOptions, options);
}
function onChildRemoved(query$1, callback, cancelCallbackOrListenOptions, options) {
	return addEventListener(query$1, "child_removed", callback, cancelCallbackOrListenOptions, options);
}
/**
...
*/
function off(query$1, eventType, callback) {
	let container = null;
...
var QueryEndAtConstraint = class extends QueryConstraint {
	...
	_apply(query$1) {
		validateFirebaseDataArg("endAt", this._value, query$1._path, true);
		const newParams = queryParamsEndAt(query$1._queryParams, this._value, this._key);
		validateLimit(newParams);
		validateQueryEndpoints(newParams);
		if (query$1._queryParams.hasEnd()) throw new Error("endAt: Starting point was already set (by another call to endAt, endBefore or equalTo).");
		return new QueryImpl(query$1._repo, query$1._path, newParams, query$1._orderByCalled);
	}
};
/**
...
var QueryEndBeforeConstraint = class extends QueryConstraint {
	...
	_apply(query$1) {
		validateFirebaseDataArg("endBefore", this._value, query$1._path, false);
		const newParams = queryParamsEndBefore(query$1._queryParams, this._value, this._key);
		validateLimit(newParams);
		validateQueryEndpoints(newParams);
		if (query$1._queryParams.hasEnd()) throw new Error("endBefore: Starting point was already set (by another call to endAt, endBefore or equalTo).");
		return new QueryImpl(query$1._repo, query$1._path, newParams, query$1._orderByCalled);
	}
};
/**
...
var QueryStartAtConstraint = class extends QueryConstraint {
	...
	_apply(query$1) {
		validateFirebaseDataArg("startAt", this._value, query$1._path, true);
		const newParams = queryParamsStartAt(query$1._queryParams, this._value, this._key);
		validateLimit(newParams);
		validateQueryEndpoints(newParams);
		if (query$1._queryParams.hasStart()) throw new Error("startAt: Starting point was already set (by another call to startAt, startBefore or equalTo).");
		return new QueryImpl(query$1._repo, query$1._path, newParams, query$1._orderByCalled);
	}
};
/**
...
var QueryStartAfterConstraint = class extends QueryConstraint {
	...
	_apply(query$1) {
		validateFirebaseDataArg("startAfter", this._value, query$1._path, false);
		const newParams = queryParamsStartAfter(query$1._queryParams, this._value, this._key);
		validateLimit(newParams);
		validateQueryEndpoints(newParams);
		if (query$1._queryParams.hasStart()) throw new Error("startAfter: Starting point was already set (by another call to startAt, startAfter, or equalTo).");
		return new QueryImpl(query$1._repo, query$1._path, newParams, query$1._orderByCalled);
	}
};
/**
...
var QueryLimitToFirstConstraint = class extends QueryConstraint {
	...
	_apply(query$1) {
		if (query$1._queryParams.hasLimit()) throw new Error("limitToFirst: Limit was already set (by another call to limitToFirst or limitToLast).");
		return new QueryImpl(query$1._repo, query$1._path, queryParamsLimitToFirst(query$1._queryParams, this._limit), query$1._orderByCalled);
	}
};
/**
...
var QueryLimitToLastConstraint = class extends QueryConstraint {
	...
	_apply(query$1) {
		if (query$1._queryParams.hasLimit()) throw new Error("limitToLast: Limit was already set (by another call to limitToFirst or limitToLast).");
		return new QueryImpl(query$1._repo, query$1._path, queryParamsLimitToLast(query$1._queryParams, this._limit), query$1._orderByCalled);
	}
};
/**
...
var QueryOrderByChildConstraint = class extends QueryConstraint {
	...
	_apply(query$1) {
		validateNoPreviousOrderByCall(query$1, "orderByChild");
		const parsedPath = new Path(this._path);
		if (pathIsEmpty(parsedPath)) throw new Error("orderByChild: cannot pass in empty path. Use orderByValue() instead.");
		const index = new PathIndex(parsedPath);
		const newParams = queryParamsOrderBy(query$1._queryParams, index);
		validateQueryEndpoints(newParams);
		return new QueryImpl(query$1._repo, query$1._path, newParams, true);
	}
};
/**
...
var QueryOrderByKeyConstraint = class extends QueryConstraint {
	...
	_apply(query$1) {
		validateNoPreviousOrderByCall(query$1, "orderByKey");
		const newParams = queryParamsOrderBy(query$1._queryParams, KEY_INDEX);
		validateQueryEndpoints(newParams);
		return new QueryImpl(query$1._repo, query$1._path, newParams, true);
	}
};
...
var QueryOrderByPriorityConstraint = class extends QueryConstraint {
	...
	_apply(query$1) {
		validateNoPreviousOrderByCall(query$1, "orderByPriority");
		const newParams = queryParamsOrderBy(query$1._queryParams, PRIORITY_INDEX);
		validateQueryEndpoints(newParams);
		return new QueryImpl(query$1._repo, query$1._path, newParams, true);
	}
};
...
var QueryOrderByValueConstraint = class extends QueryConstraint {
	...
	_apply(query$1) {
		validateNoPreviousOrderByCall(query$1, "orderByValue");
		const newParams = queryParamsOrderBy(query$1._queryParams, VALUE_INDEX);
		validateQueryEndpoints(newParams);
		return new QueryImpl(query$1._repo, query$1._path, newParams, true);
	}
};
...
var QueryEqualToValueConstraint = class extends QueryConstraint {
	...
	_apply(query$1) {
		validateFirebaseDataArg("equalTo", this._value, query$1._path, false);
		if (query$1._queryParams.hasStart()) throw new Error("equalTo: Starting point was already set (by another call to startAt/startAfter or equalTo).");
		if (query$1._queryParams.hasEnd()) throw new Error("equalTo: Ending point was already set (by another call to endAt/endBefore or equalTo).");
		return new QueryEndAtConstraint(this._value, this._key)._apply(new QueryStartAtConstraint(this._value, this._key)._apply(query$1));
	}
};
/**
...
*/
function query(query$1, ...queryConstraints) {
	let queryImpl = getModularInstance(query$1);
...
function repoManagerApplyEmulatorSettings(repo, hostAndPort, emulatorOptions, tokenProvider) {
	...
	repo.repoInfo_ = new RepoInfo(hostAndPort, useSsl, repo.repoInfo_.namespace, repo.repoInfo_.webSocketOnly, repo.repoInfo_.nodeAdmin, repo.repoInfo_.persistenceKey, repo.repoInfo_.includeNamespaceInQueryParams, true, emulatorOptions);
...
*/
function repoManagerForceRestClient(forceRestClient$1) {
	useRestClient = forceRestClient$1;
}
/**
...
function enableLogging(logger$1, persistent) {
	enableLogging$1(logger$1, persistent);
}
/**
* @license
* Copyright 2021 Google LLC
...
*/
function runTransaction(ref$1, transactionUpdate, options) {
	ref$1 = getModularInstance(ref$1);
	validateWritablePath("Reference.transaction", ref$1._path);
	if (ref$1.key === ".length" || ref$1.key === ".keys") throw "Reference.transaction failed: " + ref$1.key + " is a read-only object.";
	const applyLocally = options?.applyLocally ?? true;
	const deferred = new Deferred();
	const promiseComplete = (error$1, committed, node) => {
		let dataSnapshot = null;
		if (error$1) deferred.reject(error$1);
		else {
			dataSnapshot = new DataSnapshot(node, new ReferenceImpl(ref$1._repo, ref$1._path), PRIORITY_INDEX);
			deferred.resolve(new TransactionResult(committed, dataSnapshot));
		}
	};
	const unwatcher = onValue(ref$1, () => {});
	repoStartTransaction(ref$1._repo, ref$1._path, transactionUpdate, promiseComplete, unwatcher, applyLocally);
	return deferred.promise;
}
...
* @internal
*/
var forceRestClient = function(forceRestClient$1) {
	repoManagerForceRestClient(forceRestClient$1);
};
/**
* @license
...
*/
function _initStandalone({ app, url, version: version$1, customAuthImpl, customAppCheckImpl, nodeAdmin = false }) {
	setSDKVersion(version$1);
	/**
