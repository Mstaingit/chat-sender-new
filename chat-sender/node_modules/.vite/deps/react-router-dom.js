...
import { require_react_dom } from "./react-dom-CyfnKOhl.js";

//#region node_modules/react-router/dist/development/chunk-EF7DTUVF.mjs
		...
		if (stream || respond && matches.some((m) => m.route.unstable_middleware || typeof m.route.lazy === "object" && m.route.lazy.unstable_middleware)) {
			...
			try {
				...
				let response = await runMiddlewarePipeline({
					...
					if (stream) return await stream(requestContext, async (revalidationRequest) => {
						let result3 = await queryImpl(revalidationRequest, location$1, matches, requestContext, dataStrategy || null, skipLoaderErrorBubbling === true, null, filterMatchesToLoad || null, skipRevalidation === true);
					...
					let result2 = await queryImpl(request, location$1, matches, requestContext, dataStrategy || null, skipLoaderErrorBubbling === true, null, filterMatchesToLoad || null, skipRevalidation === true);
					...
					return await respond(renderedStaticContext);
				}, async (error, routeId) => {
	...
	async function queryRoute(request, { routeId, requestContext, dataStrategy, unstable_respond: respond } = {}) {
		...
		if (respond && matches.some((m) => m.route.unstable_middleware || typeof m.route.lazy === "object" && m.route.lazy.unstable_middleware)) {
			...
			return await runMiddlewarePipeline({
				...
				let result2 = await queryImpl(request, location$1, matches, requestContext, dataStrategy || null, false, match, null, false);
...
function loadLazyMiddlewareForMatches(matches, manifest, mapRouteProperties2) {
	let promises = matches.map(({ route }) => {
		if (typeof route.lazy !== "object" || !route.lazy.unstable_middleware) return;
...
async function defaultDataStrategyWithMiddleware(args) {
	...
	return runMiddlewarePipeline(args, false, () => defaultDataStrategy(args), (error, routeId) => ({ [routeId]: {
		type: "error",
		result: error
	} }));
}
async function runMiddlewarePipeline(args, propagateResult, handler, errorHandler) {
	...
	let middlewareState = { handlerResult: void 0 };
	try {
...
async function callRouteMiddleware(args, middlewares, propagateResult, middlewareState, handler, idx = 0) {
	...
	try {
		...
		if (nextCalled) if (result === void 0) return nextResult;
		else return result;
		else return next();
	} catch (error) {
...
async function convertDataStrategyResultToDataResult(dataStrategyResult) {
	...
	if (isResponse(result)) {
		...
		try {
			...
			if (contentType && /\bapplication\/json\b/.test(contentType)) if (result.body == null) data2 = null;
...
function StreamTransfer({ context, identifier, reader, textDecoder, nonce }) {
	...
	let scriptTag = value ? /* @__PURE__ */ import_react.createElement("script", {
		nonce,
		dangerouslySetInnerHTML: { __html: `window.__reactRouterContext.streamController.enqueue(${escapeHtml$1(JSON.stringify(value))});` }
	}) : null;
		...
		if (routeModule) {
			...
			dataRoute.lazy = {
				...
				unstable_middleware: route.hasClientMiddleware ? async () => {
					...
					invariant2$1(unstable_clientMiddleware, "No `unstable_clientMiddleware` export found");
...
//#endregion
//#region node_modules/react-router/dist/development/chunk-T3VM44WY.mjs
...
function createRoutesStub(routes, _context) {
	return function RoutesTestStub({ initialEntries, initialIndex, hydrationData, future }) {
		...
		if (routerRef.current == null) {
			...
			let patched = processRoutes(convertRoutesToDataRoutes(routes, (r) => r), _context !== void 0 ? _context : future?.unstable_middleware ? new unstable_RouterContextProvider() : {}, frameworkContextRef.current.manifest, frameworkContextRef.current.routeModules);
...
async function singleFetchLoaders(build, serverMode, staticHandler, request, handlerUrl, loadContext, handleError) {
	try {
		let respond2 = function(context) {
			let headers = getDocumentHeaders(context, build);
			if (isRedirectStatusCode(context.statusCode) && headers.has("Location")) return generateSingleFetchResponse(request, build, serverMode, {
				result: { [SingleFetchRedirectSymbol]: getSingleFetchRedirect(context.statusCode, headers, build.basename) },
				headers,
				status: SINGLE_FETCH_REDIRECT_STATUS
			});
			if (context.errors) {
		...
		if (!isResponse(result)) result = respond2(result);
		if (isRedirectResponse(result)) return generateSingleFetchResponse(request, build, serverMode, {
			result: { [SingleFetchRedirectSymbol]: getSingleFetchRedirect(result.status, result.headers, build.basename) },
			headers: result.headers,
			status: SINGLE_FETCH_REDIRECT_STATUS
		});
		return result;
...
var createRequestHandler = (build, mode) => {
	...
	return async function requestHandler(request, initialContext) {
		...
		if (_build.future.unstable_middleware) if (initialContext == null) loadContext = new unstable_RouterContextProvider();
		else try {
			loadContext = new unstable_RouterContextProvider(initialContext);
		} catch (e) {
			let error = /* @__PURE__ */ new Error(`Unable to create initial \`unstable_RouterContextProvider\` instance. Please confirm you are returning an instance of \`Map<unstable_routerContext, unknown>\` from your \`getLoadContext\` function.
		...
		if (url.pathname.endsWith(".data")) {
			...
			if (_build.entry.module.handleDataRequest) {
				...
				if (isRedirectResponse(response)) {
					...
					if (request.method === "GET") result = { [SingleFetchRedirectSymbol]: result };
...
function href(path, ...args) {
	...
	return path.split("/").map((segment) => {
		...
		if (match[2] === void 0 && value === void 0) throw Error(`Path '${path}' requires param '${param}' but it was not provided`);
		return value;
...
function createCallServer({ createFromReadableStream, createTemporaryReferenceSet, encodeReply, fetch: fetchImplementation = fetch }) {
	...
	return async (id, args) => {
		...
		const payload = await createFromReadableStream(response.body, { temporaryReferences });
		if (payload.type === "redirect") {
			if (payload.reload) {
				window.location.href = payload.location;
				return;
			}
			globalVar.__router.navigate(payload.location, { replace: payload.replace });
			return payload.actionResult;
		}
		if (payload.type !== "action") throw new Error("Unexpected payload type");
		if (payload.rerender) import_react.startTransition(async () => {
			...
			if (landedActionId < actionId && globalVar.__routerActionID <= actionId) {
				...
				if (rerender.type === "redirect") {
					...
					globalVar.__router.navigate(rerender.location, { replace: rerender.replace });
					return;
				}
				let lastMatch;
...
function getRSCSingleFetchDataStrategy(getRouter, ssr, basename, createFromReadableStream, fetchImplementation) {
	...
	return async (args) => args.unstable_runClientMiddleware(async () => {
		...
		for (const match of args.matches) {
			...
			if (renderedRoutes) for (const rendered of renderedRoutes) window.__router.patchRoutes(rendered.parentId ?? null, [createRouteFromServerManifest(rendered)], true);
		}
		return results;
	});
}
...
function RSCHydratedRouter({ createFromReadableStream, fetch: fetchImplementation = fetch, payload, routeDiscovery = "eager", unstable_getContext }) {
	if (payload.type !== "render") throw new Error("Invalid payload type");
	let router$1 = import_react.useMemo(() => createRouterFromPayload({
		payload,
		fetchImplementation,
		unstable_getContext,
	...
	return /* @__PURE__ */ import_react.createElement(RSCRouterContext.Provider, { value: true }, /* @__PURE__ */ import_react.createElement(RSCRouterGlobalErrorBoundary, { location: location2 }, /* @__PURE__ */ import_react.createElement(FrameworkContext.Provider, { value: {
		future: {
			unstable_middleware: false,
...
function RSCStaticRouter({ getPayload }) {
	const payload = import_react.use(getPayload());
	...
	return /* @__PURE__ */ import_react.createElement(RSCRouterContext.Provider, { value: true }, /* @__PURE__ */ import_react.createElement(RSCRouterGlobalErrorBoundary, { location: payload.location }, /* @__PURE__ */ import_react.createElement(FrameworkContext.Provider, { value: {
		future: {
			unstable_middleware: false,
			unstable_subResourceIntegrity: false
		},
...
function createHydratedRouter({ unstable_getContext }) {
	...
	let router2 = createRouter({
		...
		hydrationRouteProperties,
		mapRouteProperties,
		future: { unstable_middleware: ssrInfo.context.future.unstable_middleware },
...
function HydratedRouter(props) {
	if (!router) router = createHydratedRouter({ unstable_getContext: props.unstable_getContext });
